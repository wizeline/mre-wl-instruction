...
def clip_voices_and_upload(file_name, local_transcription_file, base_filename, use_voice_model, voice_model_ep, training_bucket, upload_enabled, mre_program, mre_event):
    track=1
    a_speaker_list = {}
    with open(local_transcription_file, encoding="utf8", errors='ignore') as f:
        ...
        if raw_results['results']['speaker_labels'] != None:
            ...
            for voice in raw_results['results']['speaker_labels']['segments']:
                ...
                tmp_file = tmp_file + '-' + str(voice['start_time']) + '-' + a_speaker_label + '.wav' 
                print(tmp_file)
                
                try:
                    stream = ffmpeg.input(file_name, ss=float(voice['start_time']))
                    out, err = (
                        ffmpeg.output(stream[str(track)], '/tmp/' + tmp_file, t=a_length, format='wav', ar='16000')
                    ...
                    if upload_enabled:
                        object_name = mre_program + '/' + mre_event + '/' + a_speaker_label + '/' + tmp_file 
...
def rip_audio_to_file(video_filename, track=1):
    ...
    try:
        ...
        out, err = (
            ffmpeg.output(stream[str(track)],tmp_file,format='wav',ar='16000')
            .run(capture_stdout=True, capture_stderr=True,overwrite_output=True)
        )
    except ffmpeg.Error as err:
        print(err.stderr)
...
def consolidate_transcribe_results(raw_results_file, mre_pluginhelper, a_speaker_list, speech_delay_sec=2):
    results = []
    result = {}
        ...
        for item in raw_results['results']['items']:
            ...
            if item['type'] == 'pronunciation':
                if 'Start' not in result:
                    ...
                    result['Speaker'] = get_speaker(a_speaker_list, item['speaker_label'])
...
def lambda_handler(event, context):
    ...
    try :
        ...
        training_upload_enabled = event['Plugin']['Configuration']['training_upload_enabled']
        speaker_inference_enabled = False #event['Plugin']['Configuration']['speaker_inference_enabled'] 
        voice_model_ep = 'sagemaker-mxnet-2024-03-04-18-41-13-210'
        # = str(event['Plugin']['ModelEndpoint'])
        ...
        if transcribe_lang_code is not None:
            response = transcribe_client.start_transcription_job(
                TranscriptionJobName=transcribe_job_name,
                LanguageCode=transcribe_lang_code,  
                Settings={"ShowSpeakerLabels": show_speaker_labels, "MaxSpeakerLabels": max_speaker_labels},
                MediaFormat='wav', #|'mp4'|'wav'|'flac',
                Media={
            ...
            # TODO: add the support on IdentifyMultipleLanguages = True, see docs https://docs.aws.amazon.com/transcribe/latest/dg/lang-id-batch.html#lang-id-batch-examples
            response = transcribe_client.start_transcription_job(
                TranscriptionJobName=transcribe_job_name,
                 IdentifyLanguage=True,
                 LanguageOptions = [ 'ar-SA' , 'en-US', 'es-US', 'es-ES', 'ja-JP', 'pt-BR'],
                 Settings={"ShowSpeakerLabels": show_speaker_labels, "MaxSpeakerLabels": max_speaker_labels},
                MediaFormat='wav', #|'mp4'|'wav'|'flac',
                Media={
                    'MediaFileUri': media_file_uri
        ...
        s3_client.download_file(output_bucket_name, results_file, local_transcription_file)
        
        #split audio by voice and upload to s3
        speaker_list = clip_voices_and_upload(audio_file, local_transcription_file, object_name, speaker_inference_enabled, voice_model_ep, training_bucket_name, training_upload_enabled, mre_program, mre_event)
        
        #process results
        results = consolidate_transcribe_results(local_transcription_file, mre_pluginhelper, speaker_list, silence_duration_sec)
        print(results)
